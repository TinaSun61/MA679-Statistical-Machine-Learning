---
title: "Statistical Learning Application tina"
date: "2022-12-21"
...
Your answer: The `dim()` function tells us that the data has $2930$ observations, and nine features.
...
```{r,echo=TRUE}
attach(ames_raw)
plot(`Gr Liv Area`, SalePrice)
plot(`Lot Area`, SalePrice)
plot(as.factor(Neighborhood), SalePrice)
plot(`Year Built`, SalePrice)
```
...
Your answer: `Gr Liv Area` and `Lot Area`.
...
Your answer: 
Quantitative: mpg, cylinders, displacement, housepower, weight, acceleration, year, origin, 
Qualitative: name
...
iv. Create a new qualitative variable, called Elite, by binning the Top10 perc variable. We will divide universities into two groups based on whether or not the proportion of students coming from the top 10% of their high school classes exceeds 50%.
