...
Your task is to figure out whether a patient will survive COVID-19 in Mexico.
The original dataset comes from the [Mexican government] (https://datos.gob.mx/busca/dataset/informacion-referente-a-casos-covid-19-en-mexico).  
This data is a version downloaded from [Kaggle](https://www.kaggle.com/datasets/meirnizri/covid19-dataset?resource=download).  
...
But we need to know which variable to do so before making the change.
```{r,echo=show_code}
...
Since there is very few above 100 we will look at people above 100 as 100.  
This is to make our problem easier but it also is trying to address the limit of what we can say.
...
Confusion matrix with cutoff of 0.5 looks like
```{r,echo=show_code}
...
Let's look at a decile plot.  
Decile plot looks at prediction accuracy by the predicted probability.  
Since it's easier to see the lower probability, we will look at the predicted death probability.  
You can read about Decile plots here: (https://select-statistics.co.uk/blog/cumulative-gains-and-lift-curves-measuring-the-performance-of-a-marketing-campaign/)
...
#redline: null model, proportion of patients who survived, no predictors, order patients and divided them into ten groups, from high risk to low risk, performing much better in high risk(model doing great here)
```
...
```
The model is not capturing the lower-risk patients, there is nonlinearity, and the very high-risk patients have some problems too.
These should not be a surprise, given the EDA plots we've seen so far.  
...
#Logistic
glm_fit1<- glm(
  SURVIVED_bin~AGE_TRUNK*SEX+OTHER_DISEASE,
  family=binomial,data=Train_COVID)

summary(glm_fit1)

glm_probs <- predict(glm_fit1, Test_COVID,
    type = "response")

glm_pred <- ifelse(glm_probs > 0.5,1,0)

survived.test <- Test_COVID$SURVIVED_bin

table(glm_pred , survived.test)
mean(glm_pred  == survived.test)
mean(glm_pred  != survived.test)
```
...
Your answer:
Cylinders, displacement, horsepower, weight, mpg are highly correlated with mpg01
...
```{r,echo=TRUE}
library(MASS)
lda_fit1 <- lda(mpg01 ~ displacement + horsepower + cylinders + weight, data = Auto,
    subset = train)
lda_fit1

lda_pred1 <- predict(lda_fit1, Auto[!train,])
mean(lda_pred1$class != mpg01[!train])
table(lda_pred1$class, mpg01[!train])
```
...
Your code:
```{r,echo=TRUE}
qda_fit <- qda(mpg01 ~ displacement + horsepower + cylinders + weight, data = Auto,
    subset = train)
qda_fit

qda_pred <- predict(qda_fit, Auto[!train,])
mean(qda_pred$class != mpg01[!train])
```
...
Your code:
```{r,echo=TRUE}
glm_fit <- glm(mpg01 ~ displacement + horsepower + cylinders + weight, data = Auto,
    subset = train, family=binomial)
glm_fit

glm_probs <- predict(glm_fit, Auto[!train,], type = "response")
glm_pred <-  rep(0, length(glm_probs))
glm_pred[glm_probs > 0.5] <- 1
mean(glm_pred != mpg01[!train])
```
...
```{r,echo=TRUE}
library(e1071)
nb_fit <- naiveBayes(mpg01 ~ displacement + horsepower + cylinders + weight, data = Auto,
    subset = train)
nb_fit

nb_class <- predict(nb_fit, Auto[!train,])
table(nb_class, mpg01[!train])
mean(nb_class != mpg01[!train])
```
...
```{r,echo=TRUE}
library(class)
train.X <- cbind(displacement, horsepower, cylinders, weight)[train, ]
test.X <- cbind(displacement, horsepower, cylinders, weight)[!train, ]
train.mpg01 <- mpg01[train]

set.seed(1)
knn_pred <- knn(train.X, test.X, train.mpg01, k = 5)
table(knn_pred, mpg01[!train])
mean(knn_pred != mpg01[!train])

K.collector <- rep(NA, 200)
for (k.try in 1:200){
  knn.pred <-  knn(train.X, test.X, train.mpg01, k = k.try)
  K.collector[k.try] <- mean(knn.pred != mpg01[!train])
}
x.k <- c(1:200)
...
Since we know how to take derivative of squared error loss, we can turn the perceptron problem into a liner regression problem.
$$
...
But does it work?  Let's see in an example.  Here we have a simple two class classification problem using the iris data.  The goal is to find a line that separates the two classes (setosa vs versicolor).  Setosa (blue) are the versicolor (red) are have clearly distinctive features so they are well separated by just looking at the petals.
```{r,echo=FALSE}
...
summary(fit0)
```
Result defines a surface in 3D space
...
If we draw into the figure a line that this plane and a plane at 0 intersects, we get a separating hyper plane.
...
```
So the linear model can differentiate binary outcomes.  So why do we need anything else?  The problem is that the loss function does not align with our objective.  If you look at the example below, the green and red points are separated well with a blue line.  Therefore, you are pretty sure any value above 4 is green.  Fitting the regression without one large point (very sure point) gives you the correct decision boundary.  However, with the squared error, larger values play the role of leverage point distorting the decision boundary, which contradicts what we want.  We will get back to how we will deal with this later in the semester.  But for now, let's use a loss function that is more suitable for the data.
```{r}
x=c(1,2,3,5,6,7,25)
y=c(-1,-1,-1,1,1,1,1)
par(mfrow=c(1,3))
...
- Logistic regression is a regression model for binary outcomes $y_i\in \{0,1\}$, $i = 1,\dots n$.  If we let $\pi_i$ be the probability that $y_i=1$, the interest of the modeler is to understand what factors affect this probability.  It is similar to linear regression, however, the big distinction is that $\pi_i$ is bounded between 0 and 1.  One can still fit a linear regression model, however, all the estimates will be off beyond the boundaries.
...
\end{align}
because $logit(\pi_i)$ is unbounded, this is a much better transformation for a linear model to be applied.
...
- To get the maximum likelihood we use the Newton–Raphson or Fisher scoring algorithm.
...
- The details of the Newton-Raphson method for finding the maximum likelihood goes as follows.  
...
```
    ...
    As you can see, the algorithm is starting from a guess and quickly approaches the true value, which is expressed using a dashed horizontal red line.
    
- We can generate the data many times and run the algorithm over and over.  Below we plot the trace of each algorithm.  Notice there that there is variability in the estimate due to sampling.
```{r, out.width="90%"}
par (mar=c(3,3,2,1), mgp=c(2,.7,0), tck=-.01)
set.seed(123)
kk = 20 
...
$$
which is the partial-likelihood used in logistic regression  $\prod^n_{i=1}f(y_i=1\mid \mathbf{x}_i,\boldsymbol{\beta})$ times $f(\mathbf{x}_i\mid \boldsymbol{\beta})$ term.  This term can be thought of as observed data likelihood.  If there is information about $\boldsymbol{\beta}$ in the observed data, ignoring this term result in loss of information.  Alternatively, if we factor the full likelihood as a generative model
$$
...
$$
In logistic regression with threahold at 0.5, we predict the outcome as 1 if this quantity is positive.  Where as in naive Bayes with two classes we predict 1 when the right hand side ratio is larger than 1. You can work out the details to show how they connect. Logistic regression and naive Bayes / normal discriminant analysis are what's refered to as a generative-discriminative pair. The predictive performances are comparable and depends on the data and how closely they match the assumptions of the model. You can read more about it in Ng & Jordan (2002).
